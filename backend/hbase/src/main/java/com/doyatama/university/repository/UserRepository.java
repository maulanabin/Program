...
public class UserRepository {
    ...
    public List<User> findUsersNotUsedInLectures(int size) throws IOException {
        ...
        Set<String> userIdsInStudents = new HashSet<>();
        ...
        // Get the list of all user IDs that have been used in students
        ResultScanner scannerStudent = client.getTable("students").getScanner(scan);
        for (Result result : scannerStudent) {
            byte[] userIdBytes = result.getValue(Bytes.toBytes("user"), Bytes.toBytes("id"));
            if (userIdBytes != null) {
                String userId = Bytes.toString(userIdBytes);
                userIdsInStudents.add(userId);
            }
        }
        ...
        for (User user : allUsers) {
            if (!userIdsInLectures.contains(user.getId()) && !userIdsInStudents.contains(user.getId())) {
