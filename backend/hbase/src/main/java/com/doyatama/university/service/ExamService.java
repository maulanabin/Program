...
public class ExamService {
    ...
    public PagedResponse<Exam> getAllExam(int page, int size, String studyProgramId) throws IOException {
        validatePageNumberAndSize(page, size);

        // Retrieve Polls
        List<Exam> examResponse = new ArrayList<>();

        if(studyProgramId.equalsIgnoreCase("*")) examResponse = examRepository.findAll(size);
        if(!studyProgramId.equalsIgnoreCase("*")) examResponse = examRepository.findAllByProdi(studyProgramId, size);
